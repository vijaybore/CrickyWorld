      ...
      <div style={{
        ...
        <div style={{ padding:'20px 18px 36px' }}>
          ...
          <button onClick={save} disabled={saving} style={{
            ...
            color:'#fff', fontWeight:800, fontSize:14, opacity:saving?0.7:1,
          }}>
...
export default function Settings() {
  ...
  // Read user from localStorage — set by AuthContext after OTP login
  const [user,    setUser]    = useState(() => {
    try { return JSON.parse(localStorage.getItem('user') || 'null') } catch { return null }
  })
  const [editing, setEditing] = useState(false)

  // Keep in sync if user logs in on this page
  useEffect(() => {
  ...
  return (
    <>
        ...
        <div style={{ width:'100%', maxWidth:500, minHeight:'100vh', background:'var(--surface)', display:'flex', flexDirection:'column' }}>
          ...
          <div style={{ flex:1, overflowY:'auto', padding:'20px 14px 80px', display:'flex', flexDirection:'column', gap:22 }}>
            ...
            {user ? (
                ...
                <div style={{
                  ...
                  <div style={{ display:'flex', gap:16, alignItems:'center', position:'relative' }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <div style={{ fontSize:12, color:'var(--subtext)', fontWeight:600, marginBottom:10 }}>
                      ...
                      <div style={{
                        ...
                        <span style={{ fontSize:10, color:'var(--accent)', fontWeight:800, letterSpacing:0.5 }}>SIGNED IN</span>
                      </div>
                    </div>
                  </div>
                </div>
                <Row icon="✏️" iconBg="var(--accentBg)" label="Edit Profile" sub="Change name or email" onClick={() => setEditing(true)} />
              ...
              <div style={{
                background:'var(--card)', borderRadius:20, overflow:'hidden',
                border:'1px solid var(--border)', boxShadow:'0 4px 24px var(--shadow)',
              }}>
                <div style={{
                  ...
                  <div style={{ fontFamily:'Rajdhani,sans-serif', fontSize:20, fontWeight:700, color:'var(--text)', marginBottom:6 }}>
                    Sign In to CrickyWorld
                  </div>
                  <div style={{ fontSize:12, color:'var(--subtext)', lineHeight:1.6 }}>
                    Sign in to save your stats, sync across devices and track your records
                  </div>
                </div>
      ...
      {editing && user && (
        <EditProfile user={user} onClose={() => setEditing(false)} onSaved={u => { setUser(u); localStorage.setItem('user', JSON.stringify(u)) }} />
      )}
    </>
  )
}