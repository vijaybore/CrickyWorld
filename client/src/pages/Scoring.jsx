...
function BallDot({ ball }) {
  let bg = 'var(--muted)', color = 'var(--subtext)', text = ball.runs.toString()
  if (ball.isWicket)       { bg = '#7f1d1d'; color = 'var(--accent)'; text = 'W' }
...
function PlayerPicker({ picker, pickerInput, setPickerInput, innings, striker, nonStriker, currentBowler, onSelect, onClose }) {
  ...
  return (
    <div
      style={{ position:'fixed', inset:0, background:'var(--overlay)', display:'flex', alignItems:'flex-end', justifyContent:'center', zIndex:1000 }}
      onClick={onClose}
    >
      <div
        onClick={e => e.stopPropagation()}
        style={{ width:'100%', maxWidth:500, background:'var(--card)', borderRadius:'20px 20px 0 0', padding:'20px 16px 36px', border:'1px solid var(--border)' }}
      >
        {/* handle bar */}
        <div style={{ width:40, height:4, background:'var(--text2)', borderRadius:2, margin:'0 auto 18px' }} />

        <div style={{ fontSize:12, color:'var(--subtext)', fontWeight:800, letterSpacing:1.5, marginBottom:14 }}>{title}</div>
        ...
        <div style={{ display:'flex', gap:8, marginBottom:16 }}>
          <input
            ...
            style={{ flex:1, background:'var(--surface)', border:`1px solid ${accentColor}55`, borderRadius:10, padding:'11px 14px', color:'var(--text)', fontSize:14, outline:'none' }}
          />
          <button
            onClick={() => { if (pickerInput.trim()) onSelect(pickerInput.trim()) }}
            style={{ padding:'11px 18px', borderRadius:10, background: isBowler ? '#b45309' : '#1d4ed8', border:'none', color:'var(--text)', fontWeight:800, fontSize:13, cursor:'pointer' }}
          >Set</button>
        </div>
        ...
        {filtered.length > 0 && (
          <>
            <div style={{ fontSize:10, color:'var(--subtext)', fontWeight:800, letterSpacing:1, marginBottom:8 }}>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      background: isCurrent ? `${accentColor}18` : 'var(--border2)',
                      border:`1px solid ${isCurrent ? accentColor + '55' : 'var(--border)'}`,
        ...
        {filtered.length === 0 && pickerInput.trim() && (
          <div style={{ fontSize:13, color:'var(--subtext)', textAlign:'center', padding:'12px 0' }}>
        ...
        <button
          onClick={onClose}
          style={{ marginTop:16, width:'100%', padding:'12px', borderRadius:10, background:'var(--border2)', border:'1px solid var(--border)', color:'var(--subtext)', fontWeight:700, fontSize:13, cursor:'pointer' }}
        >Cancel</button>
      </div>
    </div>
  )
    ...
    <div style={{ display:'flex', flexDirection:'column', flex:1, paddingBottom:8 }}>
      ...
      {newBatsmanModal && (
        <div style={{ position:'fixed', inset:0, background:'var(--overlay)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:2000, padding:20 }}>
          <div style={{ width:'100%', maxWidth:360, background:'var(--header)', borderRadius:20, padding:'28px 20px 24px', border:'1px solid rgba(255,68,68,0.35)', boxShadow:'0 24px 64px rgba(0,0,0,0.8)' }}>
            <div style={{ textAlign:'center', marginBottom:22 }}>
              <div style={{ fontSize:40, marginBottom:6 }}>💀</div>
              <div style={{ fontFamily:'Rajdhani,sans-serif', fontSize:22, fontWeight:700, color:'var(--accent)', letterSpacing:1 }}>WICKET!</div>
              <div style={{ fontSize:13, color:'var(--subtext)', marginTop:4 }}>{striker} — {pendingBallData?.wicketType || 'Out'}</div>
              <div style={{ fontSize:12, color:'var(--subtext)', marginTop:2 }}>Next batsman?</div>
            </div>
            <input autoFocus value={newBatsmanInput} onChange={e => setNewBatsmanInput(e.target.value)}
              ...
              style={{ width:'100%', background:'var(--surface)', border:'1.5px solid rgba(255,68,68,0.4)', borderRadius:11, padding:'13px 14px', color:'var(--text)', fontSize:15, outline:'none', boxSizing:'border-box', marginBottom:12 }}
            />
            <button onClick={handleNewBatsmanConfirm} disabled={!newBatsmanInput.trim()} style={{
              width:'100%', padding:'14px', borderRadius:11,
              background: newBatsmanInput.trim() ? 'linear-gradient(135deg,#cc0000,#ff4444)' : 'var(--muted)',
              border:'none', color: newBatsmanInput.trim() ? 'var(--text)' : 'var(--subtext)',
      ...
      {/* ── SCORE HEADER ── */}
      <div style={{ margin:'10px 12px 0', background:'linear-gradient(135deg,#1c1c1c,#1a1a1a)', border:'1px solid var(--border)', borderRadius:14, padding:'12px 16px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div>
          <div style={{ fontSize:11, color:'var(--subtext)', fontWeight:800, letterSpacing:0.5, marginBottom:2 }}>
            {innings.battingTeam} • Innings {isInnings2 ? 2 : 1}
          </div>
          <div style={{ display:'flex', alignItems:'baseline', gap:8 }}>
            <div style={{ fontFamily:'Rajdhani,sans-serif', fontSize:36, fontWeight:700, color:'var(--text)', lineHeight:1 }}>
              {innings.runs}<span style={{ fontSize:24, color:'var(--subtext)' }}>/{innings.wickets}</span>
            </div>
          </div>
        ...
        <div style={{ textAlign:'right' }}>
          <div style={{ fontSize:10, color:'var(--accent)', fontWeight:800, letterSpacing:1 }}>CRR</div>
          <div style={{ fontFamily:'Rajdhani,sans-serif', fontSize:28, fontWeight:700, color:'var(--text)' }}>
      ...
      {/* ── BATTER / BOWLER CARD ── */}
      <div style={{ margin:'8px 12px 0', background:'var(--card)', border:'1px solid var(--border)', borderRadius:12, overflow:'hidden' }}>
        {/* headers */}
        <div style={{ display:'grid', gridTemplateColumns:'1fr 38px 38px 44px', padding:'5px 14px', background:'var(--border2)', borderBottom:'1px solid var(--border2)' }}>
          <span style={{ fontSize:10, color:'var(--muted)', fontWeight:800, letterSpacing:0.5 }}>BATTER</span>
          {['R','B','SR'].map(h => <span key={h} style={{ fontSize:10, color:'var(--muted)', fontWeight:800, textAlign:'center' }}>{h}</span>)}
        </div>
        {/* striker */}
        <div onClick={() => openPicker('striker')} style={{ display:'grid', gridTemplateColumns:'1fr 38px 38px 44px', padding:'10px 14px', borderBottom:'1px solid var(--border2)', alignItems:'center', cursor:'pointer', transition:'background 0.1s' }}>
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <span style={{ width:8, height:8, borderRadius:'50%', background:'#4ade80', flexShrink:0, boxShadow:'0 0 6px #4ade8088' }}/>
            <span style={{ fontSize:14, color: striker ? 'var(--text)' : 'var(--muted)', fontWeight:800 }}>
              {striker || 'Set striker'}
              {striker && <span style={{ color:'#4ade80', fontSize:12 }}> *</span>}
            </span>
            {!striker && <span style={{ fontSize:11, color:'var(--muted)' }}>tap ✎</span>}
          </div>
          <span style={{ fontSize:15, color:'var(--text)', fontWeight:800, textAlign:'center' }}>{strikerStats?.runs ?? 0}</span>
          <span style={{ fontSize:13, color:'var(--subtext)', textAlign:'center' }}>{strikerStats?.balls ?? 0}</span>
          <span style={{ fontSize:12, color:'var(--subtext)', textAlign:'center' }}>{strikerStats?.balls > 0 ? (strikerStats.runs/strikerStats.balls*100).toFixed(0) : '—'}</span>
        </div>
        {/* non-striker */}
        <div onClick={() => openPicker('nonStriker')} style={{ display:'grid', gridTemplateColumns:'1fr 38px 38px 44px', padding:'10px 14px', borderBottom:'1px solid var(--border2)', alignItems:'center', cursor:'pointer' }}>
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <span style={{ width:8, height:8, borderRadius:'50%', background:'var(--muted)', flexShrink:0 }}/>
            <span style={{ fontSize:14, color: nonStriker ? 'var(--text2)' : 'var(--muted)', fontWeight:700 }}>
              {nonStriker || 'Set non-striker'}
            </span>
          </div>
          <span style={{ fontSize:15, color:'var(--text2)', fontWeight:800, textAlign:'center' }}>{nonStrikerStats?.runs ?? 0}</span>
          <span style={{ fontSize:13, color:'var(--subtext)', textAlign:'center' }}>{nonStrikerStats?.balls ?? 0}</span>
          <span style={{ fontSize:12, color:'var(--subtext)', textAlign:'center' }}>{nonStrikerStats?.balls > 0 ? (nonStrikerStats.runs/nonStrikerStats.balls*100).toFixed(0) : '—'}</span>
        </div>
        {/* bowler */}
        <div onClick={() => openPicker('bowler')} style={{ display:'grid', gridTemplateColumns:'1fr 38px 38px 38px 44px', padding:'8px 14px', alignItems:'center', cursor:'pointer', background:'rgba(251,146,60,0.04)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:7 }}>
            ...
            <span style={{ fontSize:14, color: currentBowler ? '#fb923c' : 'var(--muted)', fontWeight:700 }}>{currentBowler || 'Set bowler'}</span>
          </div>
          <span style={{ fontSize:12, color:'var(--subtext)', textAlign:'center' }}>{fmt(bowlerStats?.balls ?? 0)}</span>
          <span style={{ fontSize:12, color:'var(--subtext)', textAlign:'center' }}>{bowlerStats?.runs ?? 0}</span>
          <span style={{ fontSize:13, color:'var(--accent)', fontWeight:800, textAlign:'center' }}>{bowlerStats?.wickets ?? 0}</span>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 38px 38px 38px 44px', padding:'2px 14px 5px' }}>
          <span/>
          {['O','R','W','Wd'].map(l => <span key={l} style={{ fontSize:9, color:'var(--text2)', fontWeight:800, textAlign:'center' }}>{l}</span>)}
        </div>
      </div>

      {/* ── THIS OVER ── */}
      <div style={{ margin:'8px 12px 0', background:'var(--card)', border:'1px solid var(--border2)', borderRadius:12, padding:'10px 14px' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
          <span style={{ fontSize:10, color:'var(--muted)', fontWeight:800, letterSpacing:1 }}>THIS OVER</span>
          {overBalls.length > 0 && (
            <span style={{ fontSize:11, color:'var(--subtext)', fontWeight:700 }}>{overRuns} runs</span>
          )}
        </div>
        <div style={{ display:'flex', gap:6, flexWrap:'wrap', minHeight:30 }}>
          {overBalls.length === 0
            ? <span style={{ color:'var(--muted)', fontSize:12 }}>No balls bowled yet</span>
      ...
      {/* ── EXTRAS ── */}
      <div style={{ margin:'8px 12px 0', background:'var(--card)', border:'1px solid var(--border2)', borderRadius:12, padding:'10px 14px', display:'flex', gap:0, justifyContent:'space-around' }}>
        {[
          ...
          <button key={key} onClick={() => toggle(key)} style={{
            ...
            <div style={{
              ...
              border:`2px solid ${extras[key] ? color : 'var(--muted)'}`,
              background: extras[key] ? color + '33' : 'transparent',
              display:'flex', alignItems:'center', justifyContent:'center', transition:'all 0.15s'
            }}>
              {extras[key] && <span style={{ fontSize:12, color }}>✓</span>}
            </div>
            <span style={{ fontSize:10, color: extras[key] ? color : 'var(--subtext)', fontWeight:800, whiteSpace:'nowrap' }}>{label}</span>
          </button>
        ))}
      </div>
      ...
      <div style={{ margin:'8px 12px 0', display:'flex', gap:6 }}>
        {[0,1,2,3,4,5,6].map(r => {
          ...
          const c = colors[r] || { bg:'var(--accent2)', border:'var(--accent)', text:'var(--text)' }
          return (
            <button key={r} onClick={() => setRuns(r)} style={{
              flex:1, height:46, borderRadius:12,
              background: isSelected ? c.bg : 'var(--card)',
              border: `2px solid ${isSelected ? c.border : 'var(--muted)'}`,
              color: isSelected ? c.text : 'var(--subtext)',
      ...
      <div style={{ margin:'8px 12px 0', display:'flex', gap:8, position:'relative', alignItems:'stretch' }}>
        ...
        <button onClick={handleWicketToggle} style={{
          ...
          background: wicket ? 'rgba(127,29,29,0.6)' : 'var(--card)',
          border: `2px solid ${wicket ? 'var(--accent)' : 'var(--muted)'}`,
          color: wicket ? 'var(--accent)' : 'var(--subtext)',
        ...
        <div style={{ flex:1, position:'relative' }}>
          <button onClick={(e) => { e.stopPropagation(); if (wicket) setShowWicketMenu(s => !s) }} style={{
            ...
            background: wicket ? 'rgba(127,29,29,0.25)' : 'var(--card)',
            border: `2px solid ${wicket ? '#ff444444' : 'var(--muted)'}`,
            color: wicket ? '#ff9999' : 'var(--text2)',
            fontFamily:'Rajdhani,sans-serif', fontSize:14, fontWeight:700,
            cursor: wicket ? 'pointer' : 'default',
            display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 14px'
          }}>
            <span>{wicketType}</span>
            {wicket && <span style={{ fontSize:10, color:'var(--accent)' }}>▼</span>}
          </button>
          {showWicketMenu && wicket && (
            <div style={{ position:'absolute', bottom:'calc(100% + 6px)', left:0, right:0, zIndex:300, background:'var(--header)', border:'1px solid rgba(255,68,68,0.3)', borderRadius:12, overflow:'hidden', boxShadow:'0 -8px 32px rgba(0,0,0,0.8)' }}>
              {WICKET_TYPES.map(type => (
                <button key={type} onClick={() => { setWicketType(type); setAssistPlayer(''); setShowWicketMenu(false) }} style={{
                  ...
                  color: wicketType===type ? 'var(--accent)' : 'var(--text2)',
        ...
        <button onClick={handleOK} disabled={!okEnabled} style={{
          ...
          background: okEnabled ? 'linear-gradient(135deg,#cc0000,#ff4444)' : 'var(--card)',
          border: `2px solid ${okEnabled ? 'var(--accent)' : 'var(--muted)'}`,
          color: okEnabled ? 'var(--text)' : 'var(--text2)',
      ...
      {wicket && ASSIST_TYPES.includes(wicketType) && (
        <div style={{ margin:'6px 12px 0', background:'rgba(127,29,29,0.12)', border:'1px solid rgba(255,68,68,0.15)', borderRadius:11, padding:'10px 14px' }}>
          <div style={{ fontSize:10, color:'var(--accent)', fontWeight:800, letterSpacing:1, marginBottom:8 }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            <input value={assistPlayer} onChange={e => setAssistPlayer(e.target.value)}
              ...
              style={{ flex:1, background:'var(--surface)', border:'1px solid rgba(255,68,68,0.25)', borderRadius:9, padding:'8px 12px', color:'var(--text)', fontSize:13, outline:'none' }}
            />
          </div>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginTop:8 }}>
              {innings.bowlingStats.map(p => (
                <button key={p.name} onClick={() => setAssistPlayer(p.name)} style={{
                  ...
                  background: assistPlayer===p.name ? 'rgba(255,68,68,0.25)' : 'var(--border2)',
                  border: `1px solid ${assistPlayer===p.name ? '#ff444455' : 'var(--border)'}`,
                  color: assistPlayer===p.name ? '#ff9999' : 'var(--subtext)',
      ...
      <div style={{ margin:'8px 12px 0', display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8 }}>
        {[
          { label:'RETIRE',       fn: handleRetire,  color:'var(--subtext)' },
          { label:'SWITCH BAT',   fn: handleSwitchBat, color:'var(--subtext)' },
          { label:'END INNINGS',  fn: onEndInnings,  color:'var(--subtext)' },
        ].map(({ label, fn, color }) => (
      ...
      <div style={{ margin:'8px 12px 0', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
        <button onClick={onUndo} style={{
          height:40, borderRadius:10, background:'var(--card)', border:'1px solid #2a2a2a',
          color:'var(--subtext)', fontFamily:'Rajdhani,sans-serif', fontSize:13, fontWeight:700, cursor:'pointer'
        }}>⟲ UNDO</button>
        <button onClick={() => alert('Use UNDO to correct the last ball')} style={{
          ...
          color:'var(--accent)', fontFamily:'Rajdhani,sans-serif', fontSize:13, fontWeight:700, cursor:'pointer'
        }}>EDIT BALL</button>
      </div>
...
function ScorecardTab({ match }) {
  ...
  const renderInnings = (key) => {
    ...
    if (!inn || (!inn.battingStats?.length && !inn.bowlingStats?.length)) return (
      <div style={{ padding:40, textAlign:'center', color:'var(--text2)', fontSize:13 }}>No data yet</div>
    )
    ...
    const SectionHeader = ({ title, right }) => (
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 14px', background:'var(--card)', borderTop:'1px solid var(--border)', borderBottom:'1px solid var(--border)' }}>
        <span style={{ fontSize:12, fontWeight:800, color:'var(--text2)', letterSpacing:0.5 }}>{title}</span>
        {right && <span style={{ fontSize:11, color:'var(--subtext)' }}>{right}</span>}
      </div>
    )

    return (
      <div style={{ background:'var(--card)', paddingBottom:16 }}>

        {/* ── INNINGS HEADER ── */}
        <div style={{ background:'var(--card)', padding:'12px 14px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div>
            <div style={{ fontSize:12, color:'var(--text2)', fontWeight:700, marginBottom:2 }}>{key === 'innings1' ? '1st Innings' : '2nd Innings'}</div>
            <div style={{ fontSize:15, fontWeight:800, color:'var(--text)' }}>{inn.battingTeam}</div>
          </div>
          <div style={{ textAlign:'right' }}>
            <div style={{ fontFamily:'Rajdhani,sans-serif', fontSize:26, fontWeight:700, color:'var(--text)', lineHeight:1 }}>
              {inn.runs}-{inn.wickets}
            </div>
            <div style={{ fontSize:12, color:'var(--text2)', marginTop:2 }}>({fmt(inn.balls)} ov)</div>
          </div>
        </div>
        ...
        {/* col headers */}
        <div style={{ display:'grid', gridTemplateColumns:'1fr 36px 36px 32px 32px 44px 20px', padding:'5px 14px', background:'var(--header)', borderBottom:'1px solid var(--border2)' }}>
          <div style={{ fontSize:11, color:'var(--subtext)', fontWeight:700 }}>Batter</div>
          {['R','B','4s','6s','SR',''].map((h,i) => (
            <div key={i} style={{ fontSize:11, color:'var(--subtext)', fontWeight:700, textAlign:'center' }}>{h}</div>
          ))}
        </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 36px 36px 32px 32px 44px 20px', padding:'10px 14px', alignItems:'start' }}>
                ...
                <div>
                  <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom: dis ? 3 : 0 }}>
                    <span style={{ fontSize:14, color:'var(--text)', fontWeight:700 }}>{p.name}</span>
                  ...
                  {dis && (
                    <div style={{ fontSize:11, color:'var(--subtext)', lineHeight:1.4, maxWidth:180 }}>{dis}</div>
                  )}
                </div>
                <div style={{ fontSize:14, color:'var(--text)', fontWeight:800, textAlign:'center', paddingTop:2 }}>{p.runs}</div>
                <div style={{ fontSize:13, color:'var(--subtext)', textAlign:'center', paddingTop:2 }}>{p.balls}</div>
                <div style={{ fontSize:13, color:'var(--subtext)', textAlign:'center', paddingTop:2 }}>{p.fours || 0}</div>
                <div style={{ fontSize:13, color:'var(--subtext)', textAlign:'center', paddingTop:2 }}>{p.sixes || 0}</div>
                <div style={{ fontSize:12, color:'var(--subtext)', textAlign:'center', paddingTop:2 }}>{sr}</div>
                <div style={{ fontSize:13, color:'var(--text2)', textAlign:'center', paddingTop:2 }}>›</div>
              </div>
            </div>
          )
        })}
        ...
        {/* Did Not Bat */}
        {dnb.length > 0 && (
          <div style={{ padding:'8px 14px', borderBottom:'1px solid var(--border2)', background:'#181818' }}>
            <span style={{ fontSize:11, color:'var(--subtext)', fontWeight:700 }}>Did not bat: </span>
            <span style={{ fontSize:12, color:'var(--subtext)' }}>{dnb.join(', ')}</span>
          </div>
        )}

        {/* Extras row */}
        <div style={{ display:'grid', gridTemplateColumns:'1fr 36px 36px 32px 32px 44px 20px', padding:'9px 14px', background:'#181818', borderTop:'1px solid var(--border2)', borderBottom:'1px solid var(--border2)' }}>
          <div style={{ fontSize:13, color:'var(--subtext)' }}>
            Extras
            <span style={{ fontSize:11, color:'var(--text2)', marginLeft:8 }}>b 0, lb 0, w {wides}, nb {noBalls}, p 0</span>
          </div>
          <div style={{ fontSize:14, fontWeight:700, color:'var(--text)', textAlign:'center' }}>{extras}</div>
          <div/><div/><div/><div/><div/>
        </div>

        {/* ── BOWLING TABLE ── */}
        <SectionHeader title="BOWLING" />
        <div style={{ display:'grid', gridTemplateColumns:'1fr 36px 28px 36px 28px 36px', padding:'5px 14px', background:'var(--header)', borderBottom:'1px solid var(--border2)' }}>
          <div style={{ fontSize:11, color:'var(--subtext)', fontWeight:700 }}>Bowler</div>
          {['O','M','R','W','ER'].map(h => (
            <div key={h} style={{ fontSize:11, color:'var(--subtext)', fontWeight:700, textAlign:'center' }}>{h}</div>
          ))}
        </div>
        {(inn.bowlingStats || []).map((p, i) => {
          const eco = p.balls > 0 ? (p.runs/(p.balls/6)).toFixed(1) : '0.0'
          return (
            <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 36px 28px 36px 28px 36px', padding:'10px 14px', borderBottom:'1px solid var(--border2)', alignItems:'center' }}>
              <div style={{ fontSize:14, color:'var(--text)', fontWeight:600 }}>{p.name}</div>
              <div style={{ fontSize:13, color:'var(--subtext)', textAlign:'center' }}>{fmt(p.balls||0)}</div>
              <div style={{ fontSize:13, color:'var(--text2)', textAlign:'center' }}>0</div>
              <div style={{ fontSize:13, color:'var(--subtext)', textAlign:'center' }}>{p.runs||0}</div>
              <div style={{ fontSize:14, color:(p.wickets||0)>0?'var(--accent2)':'var(--subtext)', fontWeight:(p.wickets||0)>0?800:400, textAlign:'center' }}>{p.wickets||0}</div>
              <div style={{ fontSize:12, color:'var(--subtext)', textAlign:'center' }}>{eco}</div>
            </div>
          )
        })}
        ...
        {fow.length > 0 && (
          <>
            ...
            <div style={{ padding:'6px 14px 10px' }}>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0px', borderBottom:'1px solid var(--border2)', padding:'4px 0', marginBottom:2 }}>
                <span style={{ fontSize:10, color:'var(--text2)', fontWeight:700 }}>Score</span>
                <span style={{ fontSize:10, color:'var(--text2)', fontWeight:700, textAlign:'right' }}>Over</span>
              </div>
              {fow.map((f, i) => (
                <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:8, padding:'6px 0', borderBottom:'1px solid var(--border2)', alignItems:'center' }}>
                  <div style={{ fontSize:13, color:'var(--text2)' }}>
                    <span style={{ fontWeight:700 }}>{f.score}-{f.wicket}</span>
                    {f.batsmanName && <span style={{ color:'var(--subtext)', fontSize:11, marginLeft:6 }}>({f.batsmanName})</span>}
                  </div>
                  <div style={{ fontSize:12, color:'var(--subtext)', textAlign:'right' }}>{f.over} ov</div>
                </div>
              ))}
            </div>
          </>
                ...
                return (
                  <div key={i} style={{ padding:'8px 0', borderBottom:'1px solid var(--border2)' }}>
                    <div style={{ display:'grid', gridTemplateColumns:'1fr auto 1fr', gap:8, marginBottom:6, alignItems:'center' }}>
                      <div>
                        <div style={{ fontSize:13, color:'var(--text)', fontWeight:700 }}>{b1.name}</div>
                        <div style={{ fontSize:11, color:'var(--subtext)' }}>{b1.runs}({b1.balls})</div>
                      </div>
                      <div style={{ fontSize:12, fontWeight:800, color:'var(--text2)', textAlign:'center', minWidth:28 }}>{total}</div>
                      <div style={{ textAlign:'right' }}>
                        <div style={{ fontSize:13, color:'var(--text)', fontWeight:700 }}>{b2.name}</div>
                        <div style={{ fontSize:11, color:'var(--subtext)' }}>{b2.runs}({b2.balls})</div>
                      </div>
                    </div>
                    ...
                    <div style={{ height:5, borderRadius:3, overflow:'hidden', display:'flex', background:'#252525' }}>
                      ...
                      <div style={{ flex:1, background:'var(--accent2)' }}/>
                    </div>
                    <div style={{ display:'flex', justifyContent:'space-between', marginTop:3 }}>
                      <span style={{ fontSize:10, color:'#60a5fa', fontWeight:700 }}>{pct1.toFixed(0)}%</span>
                      <span style={{ fontSize:10, color:'var(--accent)', fontWeight:700 }}>{(100-pct1).toFixed(0)}%</span>
                    </div>
                  </div>
                )
              })}
        ...
        {overs.length > 0 && (
          <>
            <div
              ...
              style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'9px 14px', background:'var(--card)', borderTop:'1px solid var(--border)', borderBottom:'1px solid var(--border)', cursor:'pointer' }}
            >
              <span style={{ fontSize:12, fontWeight:800, color:'var(--text2)', letterSpacing:0.5 }}>OVER BY OVER</span>
              <span style={{ fontSize:12, color:'var(--subtext)' }}>{showOvers ? '▲ Hide' : '▼ Show'}</span>
            </div>
            {showOvers && (
              <div style={{ padding:'6px 14px 10px' }}>
                <div style={{ display:'grid', gridTemplateColumns:'44px 1fr 52px', padding:'4px 0', borderBottom:'1px solid var(--border2)', marginBottom:4 }}>
                  {['Over','Balls','Runs'].map(h => (
                    <div key={h} style={{ fontSize:10, color:'var(--text2)', fontWeight:700, textAlign: h==='Balls'?'left':'center' }}>{h}</div>
                  ))}
                </div>
                {overs.map(({ over, runs: oRuns, wickets: oWkts, balls: oBalls }, i) => (
                  <div key={i} style={{ display:'grid', gridTemplateColumns:'44px 1fr 52px', padding:'6px 0', borderBottom:'1px solid var(--border2)', alignItems:'center' }}>
                    <div style={{ fontSize:12, color:'var(--subtext)', fontWeight:700, textAlign:'center' }}>{over}</div>
                    <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>
                      {oBalls.map((b, bi) => {
                        let bg='#e8e8e8', col='var(--muted)', text=(b.runs||0).toString()
                        if (b.isWicket)      { bg='rgba(204,0,0,0.2)'; col='var(--accent)'; text='W'  }
                    ...
                    <div style={{ fontSize:13, fontWeight:700, color: oWkts>0?'var(--accent2)':'var(--text2)', textAlign:'center' }}>
                      {oRuns}{oWkts>0 && <span style={{ fontSize:11, color:'var(--accent)' }}> {oWkts}W</span>}
                    </div>
                  </div>
                ))}
              </div>
  ...
  return (
    <div style={{ flex:1, overflowY:'auto', background:'var(--header)' }}>
        ...
        <div style={{ display:'flex', background:'var(--card)', borderBottom:'1px solid var(--border)' }}>
          {[
            ...
            <button key={t.key} onClick={() => { setActiveInnings(t.key); setShowOvers(false) }} style={{
              ...
              color: activeInnings===t.key ? 'var(--accent)' : 'var(--subtext)',
              borderBottom: activeInnings===t.key ? '3px solid var(--accent)' : '3px solid transparent',
              transition:'all 0.15s'
            }}>{t.label}</button>
          ))}
        </div>
  ...
  return (
    <div style={{ padding:'12px 12px 8px', flex:1, overflowY:'auto' }}>
      ...
      <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid var(--border)', overflow:'hidden', marginBottom:12 }}>
        <div style={{ padding:'10px 14px', background:'var(--border2)', borderBottom:'1px solid var(--border2)' }}>
          <div style={{ fontSize:11, color:'var(--subtext)', fontWeight:800, letterSpacing:1 }}>SCORE COMPARISON</div>
        </div>
        {[
          ...
          <div key={i} style={{ padding:'14px 14px', borderBottom: i===0 ? '1px solid var(--border2)' : 'none' }}>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8 }}>
              <span style={{ fontSize:14, fontWeight:800, color:'var(--text)' }}>{t.team}</span>
              <span style={{ fontFamily:'Rajdhani,sans-serif', fontSize:20, fontWeight:700, color:'var(--text)' }}>
                {t.score}/{t.wkts}<span style={{ fontSize:12, color:'var(--subtext)', marginLeft:6 }}>({fmt(t.balls)})</span>
              </span>
            </div>
      ...
      <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid var(--border)', overflow:'hidden', marginBottom:12 }}>
        <div style={{ padding:'10px 14px', background:'var(--border2)', borderBottom:'1px solid var(--border2)' }}>
          <div style={{ fontSize:11, color:'var(--subtext)', fontWeight:800, letterSpacing:1 }}>KEY STATS</div>
        </div>
        {[
          ...
          <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 80px 80px', padding:'10px 14px', borderBottom:'1px solid var(--border2)', alignItems:'center' }}>
            <span style={{ fontSize:12, color:'var(--subtext)', fontWeight:700 }}>{row.label}</span>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 80px 80px', padding:'6px 14px', background:'var(--border2)' }}>
          <span/>
          <span style={{ fontSize:10, color:'var(--accent)', fontWeight:800, textAlign:'center' }}>{match.team1}</span>
      ...
      {[
        ...
        <div key={section.title} style={{ background:'var(--card)', borderRadius:14, border:'1px solid var(--border)', overflow:'hidden', marginBottom:12 }}>
          <div style={{ padding:'10px 14px', background:'var(--border2)', borderBottom:'1px solid var(--border2)' }}>
            <div style={{ fontSize:12, color:'var(--text2)', fontWeight:800 }}>{section.title}</div>
          </div>
          {section.items.length === 0
            ? <div style={{ padding:'12px 14px', color:'var(--muted)', fontSize:12 }}>No data yet</div>
            : section.items.map((p,i) => (
              <div key={i} style={{ padding:'10px 14px', display:'flex', justifyContent:'space-between', borderBottom:'1px solid var(--border2)' }}>
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  <span style={{ width:22, height:22, borderRadius:'50%', background:'rgba(255,68,68,0.15)', color:'var(--accent)', fontSize:11, fontWeight:800, display:'flex', alignItems:'center', justifyContent:'center' }}>{i+1}</span>
                  <span style={{ fontSize:13, color:'var(--text)', fontWeight:700 }}>{p.name}</span>
                </div>
                <span style={{ fontFamily:'Rajdhani,sans-serif', fontSize:16, fontWeight:800, color:'var(--text)' }}>
                  {p[section.key]} <span style={{ fontSize:11, color:'var(--subtext)' }}>{section.suffix}</span>
                </span>
              </div>
            ))
          }
...
export default function Scoring() {
  ...
  if (fetching) return (
    <div style={{ minHeight:'100vh', background:'var(--surface)', display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ color:'var(--accent)', fontFamily:'Rajdhani,sans-serif', fontSize:20 }}>Loading match...</div>
    </div>
  )
  if (error) return (
    <div style={{ minHeight:'100vh', background:'var(--surface)', display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:16 }}>
      <div style={{ color:'var(--accent)', fontSize:16 }}>{error}</div>
      <button onClick={() => navigate('/')} style={{ padding:'10px 24px', background:'var(--accent2)', border:'none', borderRadius:10, color:'var(--text)', fontWeight:700, cursor:'pointer' }}>Go Home</button>
    </div>
  )
    ...
    <>
      ...
      <div style={{ minHeight:'100vh', width:'100%', background:'var(--bg)', display:'flex', justifyContent:'center' }}>
        <div style={{ width:'100%', maxWidth:500, minHeight:'100vh', background:'var(--surface)', display:'flex', flexDirection:'column' }}>
          ...
          {/* ── Top Header ── */}
          <div style={{ padding:'14px 16px 12px', flexShrink:0, background:'linear-gradient(180deg,#1a1a1a 0%,transparent 100%)', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              <button onClick={() => navigate('/')} style={{ width:34, height:34, borderRadius:9, background:'var(--border)', border:'1px solid var(--border)', color:'var(--text2)', fontSize:16, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>←</button>
              <div>
                <div style={{ fontFamily:'Rajdhani,sans-serif', fontSize:18, fontWeight:700, color:'var(--text)', letterSpacing:1 }}>
                  {match.team1} <span style={{ color:'var(--accent)', fontSize:14 }}>vs</span> {match.team2}
                </div>
            ...
            <button onClick={handleUndo} style={{ padding:'6px 12px', borderRadius:8, background:'var(--border2)', border:'1px solid var(--border)', color:'var(--text2)', fontFamily:'Rajdhani,sans-serif', fontSize:13, fontWeight:700, letterSpacing:1, cursor:'pointer' }}>UNDO</button>
          </div>
          ...
          {/* ── Bottom Nav ── */}
          <div style={{ flexShrink:0, display:'flex', background:'var(--header)', borderTop:'1px solid var(--border)', paddingBottom:'env(safe-area-inset-bottom,0px)' }}>
            {tabs.map(t => (
              <button key={t.key} onClick={() => setTab(t.key)} style={{ flex:1, padding:'12px 0', border:'none', cursor:'pointer', background:'transparent', display:'flex', flexDirection:'column', alignItems:'center', gap:3, position:'relative', transition:'background 0.15s' }}>
                ...
                <span style={{ fontSize:10, fontWeight:800, letterSpacing:0.5, color: tab === t.key ? 'var(--accent)' : 'var(--muted)', transition:'color 0.15s' }}>{t.label}</span>
              </button>
            ))}
          </div>
